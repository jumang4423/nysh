...
pub fn update_commands(val: String) {
    println!("{:?}", val);
    // esto no lo deberia de hacer ni por aaccidente pero no se por que no funciona
    let sql = "UPDATE commands SET last_time_used=strftime('%f', 'now')+ strftime('%s', 'now') WHERE command=?1;";
    let conn = sqlite::open("src/database/database.db").unwrap();
    let stm = conn.prepare(sql).unwrap();

    stm.into_cursor().bind(&[Value::String(val)]).unwrap();
}
